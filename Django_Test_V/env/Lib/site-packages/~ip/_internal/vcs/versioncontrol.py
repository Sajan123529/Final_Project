...
def find_path_to_setup_from_repo_root(location, repo_root):
    ...
    """
    Find the path to `manage.py` by searching up the filesystem from `location`.
    Return the path to `manage.py` relative to `repo_root`.
    Return None if `manage.py` is in `repo_root` or cannot be found.
    """
    # find manage.py
    orig_location = location
    while not os.path.exists(os.path.join(location, 'manage.py')):
        ...
        if location == last_location:
            ...
            # finding manage.py
            logger.warning(
                "Could not find manage.py for directory %s (tried all "
                "parent directories)",
                orig_location,
            )
            return None
...
class VersionControl:
    ...
    def get_subdirectory(cls, location):
        ...
        """
        Return the path to manage.py, relative to the repo root.
        Return None if manage.py is in the repo root.
        """
        return None
